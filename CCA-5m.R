...
spe5 <- read.delim("E:/小论文/东印度洋相关信息/分析方法/CCA分析/5-species.csv", 
                  row.names = 1, sep = ',', stringsAsFactors = FALSE, check.names = FALSE)
env5 <- read.delim("E:/小论文/东印度洋相关信息/分析方法/CCA分析/5-environment.csv", 
                  row.names = 1, sep = ',', stringsAsFactors = FALSE, check.names = FALSE)

scale.env5 <- scale (env5, center = TRUE, scale = TRUE)#执行scale是为了消除量纲影响
spp=decostand(spe5,method = "hellinger")#0数据多，或想消除极值影响,做h转化

otu_cca <- cca (spp, scale.env5)# 进行分析
